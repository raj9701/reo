...
app.controller('AppCtrl', function($scope, $ionicPopup, $http,$state,$stateParams){ 
   $http({
        method: "GET",
        url: "https://peaceful-meadow-62073.herokuapp.com/contactlists",
        //crossDomain : true,
   ...
   confirmPopup.then(function(res) {
     if(res) {
       ...
       //console.log('You are sure');
        $http.delete('https://peaceful-meadow-62073.herokuapp.com/contactlist/'+id).then(function(response){          
...
//  $scope.showPopup = function() {
  ...
  myPopup.then(function(res) {
    ...
    if(res){
      $http.post('https://peaceful-meadow-62073.herokuapp.com/contactlist',$scope.contact).then(function(response){                   
            $scope.contact = "";
            $state.go('list');    
            //A Alert Popup
              var alertPopup = $ionicPopup.alert({
...
app.controller('AddCtrl', function($scope, $stateParams, $http, $state){
    $scope.adduser = function(contact){
         console.log(contact);
         $http.post('https://peaceful-meadow-62073.herokuapp.com/contactlist',contact).then(function(response){                   
            $scope.contact = "";
            $state.go('list');                             
          });  
       };
...
app.controller('EditCtrl', function($scope, $stateParams, $http, $state,$ionicPopup){
     var id = $stateParams.userId;
        console.log(id);            
    $http.get('https://peaceful-meadow-62073.herokuapp.com/contactlist/'+id).then(function(response){
      $scope.contact = response.data;      
    });  

    $scope.update = function(){
      console.log($scope.contact._id);
      $http.put('https://peaceful-meadow-62073.herokuapp.com/contactlist/' + $scope.contact._id, $scope.contact).then(function(response){      
       $state.go('list'); 
          //A Alert Popup
              var alertPopup = $ionicPopup.alert({
